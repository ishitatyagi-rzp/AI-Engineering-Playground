# dev-playground — Codex Configuration

This repo is used for system design problem implementations driven by AI.

## Rules

Detailed rules are in `.Codex/rules/`. Follow all of them on every task.

- `.Codex/rules/code-structure.md` — layering, naming, clean code
- `.Codex/rules/testing.md` — TDD, test pyramid, mocking
- `.Codex/rules/error-handling.md` — logging, wrapping, no silent swallows
- `.Codex/rules/api-design.md` — contract-first, validation, versioning
- `.Codex/rules/security.md` — input validation, no credentials in code
- `.Codex/rules/database.md` — indexes, timestamps, query patterns
